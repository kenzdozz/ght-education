...
const Service = () => {
    const renderServices = useCallback(
        ({ text, icon, id }: { text: string, icon: SVGIconName, id: number }) => {
            return (
                <AnimateScrollReveal duration={1.2} delay={id * 0.115} allowOverflow key={id} className={`shrink-0 relative w-full h-32 px-2 flex flex-col justify-center items-center gap-2 rounded-md ${S.ServiceItem}`}>

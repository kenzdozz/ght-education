...
const Header = () => {
    const renderProcess = useCallback(
        ({ icon, head, text }: { icon: SVGIconName, head: string, text: string }, i: number) => {
            return (
                <AnimateScrollReveal delay={i * 0.1} duration={i * 0.112} key={i} className=' w-full flex flex-col px-2 py-8 md:px-5 justify-center items-center header-home glass cursor-pointer'>
    ...
    return (
        <Container className={`container relative overflow-x-clip bg-transparent ${S.headPsuedo}`}>
            <Container className={`container-inner relative z-[10] ${S.headPsuedo}`}>
                <AnimateScrollReveal duration={1.75} animName="fadeInRight" as="h1" className=" text-slate-700 w-full lg:w-2/3 font-extrabold text-4xl md:text-7xl">
                    Travel To Your Dream Country Through <Container as='span' className="text-gradient">GlobalHT Education</Container>
                </AnimateScrollReveal>
            </Container>
            <Container className=' hidden lg:block rounded-[9rem] w-[24rem] h-[45rem] animate-fading-pulse bg-gradient-primary-bottom absolute z-[0] -right-4 -top-10 -rotate-[40deg] overflow-clip'>
                <Container className='flex items-start justify-start translate-x-3 w-[98%] h-full relative z-[0]'>
                    <Image
                        src={`/img/hero1.webp`}
                        alt="Your Image"
            ...
            <Container className={`container-inner mt-10 ${S.headBottomPsuedo}`}>
                ...
                <AnimateScrollReveal allowOverflow className=' relative z-[2] grid grid-cols-2 md:grid-cols-4 gap-3 mt-5'>
                    {

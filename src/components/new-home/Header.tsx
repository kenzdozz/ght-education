        ...
        ({ icon, head, text }: { icon: SVGIconName, head: string, text: string }, i: number) => {
            return (
                <AnimateScrollReveal
                    ...
                    className={` w-full flex flex-col px-2 py-8 md:px-5 justify-center items-center header-home glass cursor-pointer ${S.bgWhite50t}`}>
                    <Container as='span' className=' text-blue-800'>
                        <SVGIcon name={icon} size='large' />
                    </Container>
                    <Container className=' uppercase text-sm lg:text-base font-semibold text-red-600 text-center mb-2'>{head}</Container>
        ...
        <Container className={`relative overflow-x-clip bg-transparent ${S.headPsuedo}`}>
            <Container className={`container relative ${S.headPsuedo}`}>
                <AnimateScrollReveal duration={1.75} animName="fadeInRight" as="h1" className=" text-slate-700 w-full lg:w-2/3 font-extrabold text-5xl md:text-7xl">
                    Study In Your Dream Country Through <Container as='span' className="text-gradient">GlobalHT Education</Container>
                </AnimateScrollReveal>
            </Container>
